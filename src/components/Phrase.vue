<template>
  <div class="phrase">
    ...
    <div class="text-center">
      ...
      <button
        class="button-audio"
        v-if="shouldPlayAudio && isPhraseVisible"
        v-on:click="onPlay"
      >
        <i class="fas fa-volume-up"></i>
      </button>
      <div class="space" />
      <button class="button-continue" v-on:click="onContinue">
        {{ isPhraseVisible ? "Continue" : "Show phrase" }}
      </button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    initAudio: function() {
      ...
      audio.src = `https://ssl.gstatic.com/dictionary/static/sounds/oxford/${this.parseAudioString(
        this.phrase.phrase
      )}--_gb_1.mp3`;
    },
    resetAudio: function() {
    ...
    parseAudioString: function(phrase) {
      ...
      return phraseWithoutBrackets.split(" ")[0].toLowerCase();
    },
    playAudio: function() {
      if (this.shouldPlayAudio && this.audio) this.audio.play();
    },
    onContinue: function() {
